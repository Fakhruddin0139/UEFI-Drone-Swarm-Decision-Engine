...
#define PRIORITY_PAYLOAD 90 // Never sacrifice unless critical
#define PRIORITY_ESCORT 50  // Mid-level protection
#define PRIORITY_SCOUT 20   // First to sacrifice

// ── Drone health status thresholds ──────────────────────────
#define HEALTH_CRITICAL 25 // Below this = high sacrifice risk
#define HEALTH_DAMAGED 50  // Below this = moderate risk
#define HEALTH_GOOD 75     // Above this = low risk

// ── Swarm configuration ──────────────────────────────────────
#define MAX_SWARM_SIZE 5 // Maximum drones in swarm

// ── SWARM_DRONE Struct (Final locked version) ────────────────
// Phase 2: Drone Swarm Decision Engine
typedef struct
{
    CHAR16 DroneID[9];        // L"DRONE-A", L"DRONE-B" etc
    GNSS_DATA Position;       // Live coordinates from GNSS
    INTN Health;              // 0-100
    INTN Fuel;                // 0-100
    INTN Weapons;             // 0-10
    INTN Priority;            // Mission role importance
                              // 90=Payload, 50=Escort, 20=Scout
    INTN MissionValue;        // Calculated score (updated each cycle)
    INTN DistanceToTarget;    // Meters to waypoint
    INTN BaseMotorCurrent;    // Baseline motor current before damage
    INTN CurrentMotorCurrent; // Live motor current reading
    INTN BaseDrag;            // Baseline drag before damage
    INTN CurrentDrag;         // Live drag reading
    INTN LastHeartbeatTick;   // Watchdog timer
    BOOLEAN DamageDetected;   // TRUE if hit detected
    BOOLEAN IsAlive;          // FALSE = dead, excluded from all logic
    BOOLEAN SacrificeFlag;    // TRUE = this drone intercepts threat
} SWARM_DRONE;
...
INTN CalculateMissionValue(SWARM_DRONE *Drone)
{
    ...
    return (Drone->Health * 38) + (Drone->Weapons * 30) + (Drone->Fuel * 22) + (Drone->Priority * 35);
}
...
VOID DetectDamage(SWARM_DRONE *Drone, INTN MotorCurrent, INTN DragValue)
{
    ...
    INTN DragChange = DragValue - Drone->BaseDrag; // ← Fixed
    ...
    INTN LonMultiplier)
{
    ...
    INTN CurrentLatTHM = FlattenToTHM(
        Drone->Position.LatDeg, Drone->Position.LatMin, Drone->Position.LatSec);

    INTN CurrentLonTHM = FlattenToTHM(
        Drone->Position.LonDeg, Drone->Position.LonMin, Drone->Position.LonSec);
    ...
    INTN LonMultiplier)
{
    ...
    // Step 1: Reset + Update all drones
    for (INTN i = 0; i < SwarmSize; i++)
    {
        Swarm[i].SacrificeFlag = FALSE;
    ...
    // Step 2: Find drone with lowest MissionValue (most expendable)
    for (INTN i = 0; i < SwarmSize; i++)
    {
        if (!Swarm[i].IsAlive)
            continue;
        ...
        {
            // Tie-breaker: Closer drone to threat sacrifices
...
VOID DisplaySwarmStatus(SWARM_DRONE Swarm[], INTN SwarmSize, INTN SacrificialIndex)
{
    ...
    for (INTN i = 0; i < SwarmSize; i++)
    {
        ...
        Print(L"| %-6s | %3d%%   | %3d%% | %5d | %8d | %-10s |\r\n",
              D->DroneID, D->Health, D->Fuel, D->Weapons, D->Priority, StatusText);
    }

    Print(L"+--------+--------+------+-------+----------+----------+\r\n\n");

    // Mission Value + Distance Table
    Print(L"+--------+---------------+------------------+\r\n");
    ...
    for (INTN i = 0; i < SwarmSize; i++)
    {
        ...
        {
            Print(L"| %-6s | %13d | %16d |\r\n",
                  D->DroneID, D->MissionValue, D->DistanceToTarget);
        }
    }
    Print(L"+--------+---------------+------------------+\r\n\n");
...
// ── Initialize swarm with 5 drones of different roles ────────
VOID InitializeSwarm(SWARM_DRONE Swarm[], INTN *SwarmSize)
{
    *SwarmSize = 5;

    // DRONE-A: Payload Carrier (High Priority)
    StrCpyS(Swarm[0].DroneID, 9, L"DRONE-A");
    Swarm[0].Health = 85;
    Swarm[0].Fuel = 80;
    Swarm[0].Weapons = 5;
    Swarm[0].Priority = PRIORITY_PAYLOAD;
    Swarm[0].BaseMotorCurrent = 100;
    Swarm[0].CurrentMotorCurrent = 100;
    Swarm[0].BaseDrag = 50;
    Swarm[0].CurrentDrag = 50;
    Swarm[0].LastHeartbeatTick = 0;
    Swarm[0].DamageDetected = FALSE;
    Swarm[0].IsAlive = TRUE;
    Swarm[0].SacrificeFlag = FALSE;
    Swarm[0].MissionValue = 0;
    Swarm[0].DistanceToTarget = 0;
    Swarm[0].Position.LatDeg = 22;
    Swarm[0].Position.LatMin = 32;
    Swarm[0].Position.LatSec = 60;
    Swarm[0].Position.LatDir = 'N';
    Swarm[0].Position.LonDeg = 88;
    Swarm[0].Position.LonMin = 22;
    Swarm[0].Position.LonSec = 70;
    Swarm[0].Position.LonDir = 'E';
    ...
    Swarm[1].BaseMotorCurrent = 100;
    Swarm[1].CurrentMotorCurrent = 100;
    Swarm[1].BaseDrag = 50;
    Swarm[1].CurrentDrag = 50;
    Swarm[1].LastHeartbeatTick = 0;
    Swarm[1].DamageDetected = FALSE;
    Swarm[1].IsAlive = TRUE;
    Swarm[1].SacrificeFlag = FALSE;
    Swarm[1].MissionValue = 0;
    Swarm[1].DistanceToTarget = 0;
    Swarm[1].Position.LatDeg = 22;
    Swarm[1].Position.LatMin = 32;
    Swarm[1].Position.LatSec = 55;
    Swarm[1].Position.LatDir = 'N';
    Swarm[1].Position.LonDeg = 88;
    Swarm[1].Position.LonMin = 22;
    Swarm[1].Position.LonSec = 65;
    Swarm[1].Position.LonDir = 'E';
    ...
    Swarm[2].BaseMotorCurrent = 100;
    Swarm[2].CurrentMotorCurrent = 125;
    Swarm[2].BaseDrag = 50;
    Swarm[2].CurrentDrag = 68;
    Swarm[2].LastHeartbeatTick = 0;
    Swarm[2].DamageDetected = TRUE;
    Swarm[2].IsAlive = TRUE;
    Swarm[2].SacrificeFlag = FALSE;
    Swarm[2].MissionValue = 0;
    Swarm[2].DistanceToTarget = 0;
    Swarm[2].Position.LatDeg = 22;
    Swarm[2].Position.LatMin = 32;
    Swarm[2].Position.LatSec = 65;
    Swarm[2].Position.LatDir = 'N';
    Swarm[2].Position.LonDeg = 88;
    Swarm[2].Position.LonMin = 22;
    Swarm[2].Position.LonSec = 75;
    Swarm[2].Position.LonDir = 'E';
    ...
    Swarm[3].BaseMotorCurrent = 100;
    Swarm[3].CurrentMotorCurrent = 100;
    Swarm[3].BaseDrag = 50;
    Swarm[3].CurrentDrag = 50;
    Swarm[3].LastHeartbeatTick = 0;
    Swarm[3].DamageDetected = FALSE;
    Swarm[3].IsAlive = TRUE;
    Swarm[3].SacrificeFlag = FALSE;
    Swarm[3].MissionValue = 0;
    Swarm[3].DistanceToTarget = 0;
    Swarm[3].Position.LatDeg = 22;
    Swarm[3].Position.LatMin = 32;
    Swarm[3].Position.LatSec = 50;
    Swarm[3].Position.LatDir = 'N';
    Swarm[3].Position.LonDeg = 88;
    Swarm[3].Position.LonMin = 22;
    Swarm[3].Position.LonSec = 60;
    Swarm[3].Position.LonDir = 'E';
    Swarm[3].Position.Valid = TRUE;

    // DRONE-E: Critical Scout (most likely to sacrifice)
    StrCpyS(Swarm[4].DroneID, 9, L"DRONE-E");
    Swarm[4].Health = 25;
    Swarm[4].Fuel = 30;
    Swarm[4].Weapons = 1;
    Swarm[4].Priority = PRIORITY_SCOUT;
    Swarm[4].BaseMotorCurrent = 100;
    Swarm[4].CurrentMotorCurrent = 100;
    Swarm[4].BaseDrag = 50;
    Swarm[4].CurrentDrag = 50;
    Swarm[4].LastHeartbeatTick = 0;
    Swarm[4].DamageDetected = FALSE;
    Swarm[4].IsAlive = TRUE;
    Swarm[4].SacrificeFlag = FALSE;
    Swarm[4].MissionValue = 0;
    Swarm[4].DistanceToTarget = 0;
    Swarm[4].Position.LatDeg = 22;
    Swarm[4].Position.LatMin = 32;
    Swarm[4].Position.LatSec = 70;
    Swarm[4].Position.LatDir = 'N';
    Swarm[4].Position.LonDeg = 88;
    Swarm[4].Position.LonMin = 22;
    Swarm[4].Position.LonSec = 80;
    Swarm[4].Position.LonDir = 'E';
...
VOID DroneSwarmMenu()
{
    ...
    while (Running)
    {
        ...
        case 1:
        {
            // Show current swarm status — no threat
        ...
        case 2:
        {
            // Simulate bullet hit on a specific drone
            gST->ConOut->ClearScreen(gST->ConOut);
            ...
            if (Target >= 1 && Target <= (UINTN)SwarmSize)
            {
                ...
                {
                    // Simulate major bullet hit
                    Swarm[Idx].CurrentMotorCurrent = 130; // Update live sensor
                    Swarm[Idx].CurrentDrag = 85;          // Update live sensor

                    DetectDamage(&Swarm[Idx], 130, 85); // Now calculate damage
        ...
        case 3:
        {
            // Simulate missile threat — run sacrifice engine
            gST->ConOut->ClearScreen(gST->ConOut);
            Print(L"\n  !! MISSILE THREAT DETECTED !!\n");
            Print(L"  Running sacrifice decision engine...\n");
            gBS->Stall(1500000); // 1.5 second dramatic pause
        ...
        case 4:
        {
            ...
            // Update heartbeats for all alive drones
            for (INTN i = 0; i < SwarmSize; i++)
